...
//starts from end of string and removes all special characters until alphanumeric ex: file!! -> goes to file, fi!le -> fi!le
void remove_end_special_characters(char *string){
	...
	if(input == NULL){
	   ...
	   //reading input file
		while(fscanf(input,"%s",input_txt)!= EOF){
		   ...
		   //remove special character punctation from end of word
		   remove_end_special_characters(input_txt);
		...
		//writing output file
		FILE *output = fopen("out_improved.txt", "w");
